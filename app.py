...
@app.route('/add_interaction/<int:person_id>/<string:contact_name>', methods=['POST', 'GET'])
...
@app.route('/update_interaction/<int:interaction_id>/<int:person_id>/<string:contact_name>', methods=['POST', 'GET'])
def update_interaction(interaction_id, person_id, contact_name):
    # Fetch interaction info from database to prefil edit form
    if request.method == 'GET':
        with mysql.connector.connect(**config) as cnx:
            with cnx.cursor(buffered=True) as cursor:
